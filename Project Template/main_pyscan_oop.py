...
@History:
2019-0910 Peter - OOP version, add lopy4_board abd pyscan_board specification
...
class App:
    ...
    def VALID_CARDS(self):
        return [[0x43, 0x95, 0xDD, 0xF8],
                [0x43, 0x95, 0xDD, 0xF9],
                [0x11, 0x22, 0x33, 0x44],   # add your own card ID in hex
                [0xAA, 0xBB, 0xCC, 0xDD]]   # add your own card ID in hex
    ...
    def send_sensor_data(self, name, timeout):
        while(True):
            print('Lightsensor: {}'.format(self.lt.light()))  # 2019 changed
            print('Accelerometer: {}'.format(self.li.acceleration()))  # 2019 changd
            time.sleep(timeout)
